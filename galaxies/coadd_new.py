...
def coadd_truth(filename):
    ...
    gals = fits.open(name)
    data = gals[1].data
    data = data[data['gal_star'] == 0]
    #print(len(data[data['mag'][:,3] != 0]))
    return data[data['mag'][:,3] != 0]

def count_images(ra_min,ra_max,dec_min,dec_max,ra,dec,filename):
    #if ra < ra_min or ra > ra_max or dec < dec_min or dec > dec_max:
        #return 0
    x = np.array((ra - ra_min) / (ra_max - ra_min) * 8820).astype(int)
    y = np.array((dec - dec_min) / (dec_max - dec_min) * 8820).astype(int)
    ...
    mask = np.all([x > 0,x < 8822, y>0,y<8822], axis=0)
    output = np.zeros_like(x)
    output[mask] = cc[x[mask],y[mask]]
    return output


for f in glob.glob('paper_final_Y*.cat'):
    detected = 0
    coadd_imagefile = '../' + f[:-4] + '.fits'
    cfile = f
    ...
    image_count = count_images(ra_min, ra_max, dec_min, dec_max, data['ra'], data['dec'], coadd_imagefile)
    for i, gal in enumerate(galcoords):
        ...
        if data[i]['mag'][3] < 28:
            if galid not in gals:
                gals[galid] = {'total': 1, 'found': detected, 'galmag': data[i]['mag'][3], 'image_count': bin(image_count[i]).count('1')}
            else:
...
for v in gals.values():
    if float(v['found']) / float(v['total']) >= 0.5:
        fimages = np.append(fimages, v['image_count'])
    else:
        nfimages = np.append(nfimages, v['image_count'])
plt.hist(fimages, histtype='step', label='Found Galaxies')
plt.hist(nfimages, histtype='step', label='Not Found')
plt.xlabel('Number of images')
plt.legend()
plt.savefig('ImageNumberComparison')
plt.clf()
