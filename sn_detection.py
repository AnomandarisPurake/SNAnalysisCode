        ...
        for i in range(len(data)):
            ...
            if data[i]['mag'] < 30:
                ...
                if len(gal) != 0:
                    ...
                    if galsndist > 1.5 * galdata[gal['ind'][0]]['size'] and gal['mag'] > 23:
...
plt.hist((found,np.append(found,notfound)), histtype = 'step',stacked=False,label=('Images with detections','Total images'),bins=30)
...
plt.savefig('SNEfficiency.png')
plt.clf()

single_found = np.array([])
single_notfound = np.array([])
for v in sne.values():
    if v['found'] >= 1:
        single_found = np.append(single_found, v['peakmag'])
    else:
        single_notfound = np.append(single_notfound, v['peakmag'])
plt.hist((single_found,single_notfound), histtype = 'step',stacked=True,label=('Detected supernovae','Not detected supernovae'),bins=30)
plt.xlabel('Supernova peak magnitude')
plt.ylabel('Number of supernovae')
plt.legend()
plt.savefig('SingleSNFound.png')
