...
sne = dict()
for f in glob.glob('coadds/paper_final_Y*.cat'):
    detected = 0
    cfile = f
...
plt.scatter(cgrs,cwrs,label='Host correctly identified - 411/489') 
plt.scatter(fgrs,fwrs,label='Host detected, not correctly identified - 8/489')
plt.scatter(nfgrs,nfwrs,label='Host not detected - 70/489') 
plt.xlabel('True z')
